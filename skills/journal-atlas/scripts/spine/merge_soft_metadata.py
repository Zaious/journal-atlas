#!/usr/bin/env python3
"""
Merge AI-research (soft-metadata AI-research) drafts + the spine (journal_spine.db) into
...
are hand-curated Tier 1 entries with subjective content AI-research has no equivalent
for). This script only CREATES new entries.

Design: renders the fixed TEMPLATE.md v1.3 structure. Fields with no data in
either the spine or the AI-research draft are marked "(pending)" — same convention as
existing entries — rather than invented. New entries get a distinct banner
("AI-Researched, AI-research") instead of "Tier 1" / "Tier 2 (community estimate)",
...
MAX_SLUG_LEN = 80  # guards against Windows MAX_PATH when a AI-research draft's "name" field
...
> Source: OpenAlex + JUFO + CAS 中科院分区 + Norwegian Register (spine, snapshot {spine.get('built_on', today)}) + AI research (experiential facts, cited per-field below).
...
| **CiteScore** | *(pending)* | |
| **Acceptance Rate** | {acceptance_note} | AI research (see Reviewer Pool Characteristics sources) |
| **Desk Rejection Rate** | {desk_reject} | AI research |
...
| **Time to acceptance (total)** | {review_time} | AI research (see sources below); may be a range/qualitative note, not a precise figure |
...
> AI-research note (not broken down per-stage): {preprint_note}
...
**Current positioning (what the journal accepts now):** {accepts_now}

**Methods noted as welcome (qualitative — not a scored 0-5 table, see Methodological Preferences below):** {methods_line}
...
- **Framing requirement noted by AI research:** {framing}
...
| Date | Change | By |
|------|--------|----|
| {today} | Auto-generated: Identity/Metrics/Subject Density from spine (OpenAlex + JUFO + CAS + Norwegian Register + DOAJ, snapshot {spine.get('built_on', today)}); Policies/Positioning/Experiential Soft Metadata from AI research (signal_quality {fmt(sq,'0')}/5). Subjective Soft Metadata subsections (political leanings, sensitive topics, voice/style, scored methodology preferences) and Strategic Notes intentionally left pending — no evidentiary basis collected by this pipeline. | @Zaious |
"""
