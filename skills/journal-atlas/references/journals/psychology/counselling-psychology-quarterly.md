<!-- schema: v1.3 -->

# Counselling Psychology Quarterly

> **Last verified**: 2026-05-17
> **Maintainer**: @Zaious
> **Venue type**: Journal

---

## Identity

| Field | Value |
|-------|-------|
| **Abbreviation** | Couns Psychol Q |
| **Venue type** | Journal |
| **Publisher** | Taylor & Francis (Routledge) |
| **ISSN (Print)** | 0951-5070 |
| **ISSN (Online)** | 1469-3674 |
| **Founded** | 1988 |
| **URL** | https://www.tandfonline.com/journals/ccpq20 |
| **Author / Submission Guidelines** | https://www.tandfonline.com/action/authorSubmission?journalCode=ccpq20 |
| **Society / Association** | BACP-adjacent (British Association for Counselling and Psychotherapy) |
| **Editorial Board Location** | International (UK-anchored) |

---

## Metrics

> Source: publisher / JCR / community trackers. Retrieved 2026-05-17.

| Metric | Value | Date |
|--------|-------|------|
| **Impact Factor** | 1.5 (2024 JCR) | 2026-05-17 |
| **5-Year IF** | *(see JCR)* | |
| **h-index** | ~50+ | 2026-05-17 |
| **CiteScore** | *(see Scopus)* | |
| **Acceptance Rate** | ~21% (T&F official, journal metrics page) | 2026-07-13 |
| **Desk Rejection Rate** | *(community estimate)* | |
| **Quartile (by field)** | *(see SCImago)* | |

### Review Cycle Time

| Stage | Typical Time | Notes |
|-------|-------------|-------|
| **Time to first decision** | ~11 days (T&F official) | Avg. submission-to-first-decision per publisher's journal metrics page; may include editorial-screening decisions |
| **Time to first review** | ~125 days (T&F official) | Avg. submission-to-first-post-review-decision per publisher's journal metrics page; publisher's own label, may not map 1:1 onto 'first review' if R&R rounds are involved |
| **Time to acceptance (total)** | *(community estimate)* | |
| **Time to publication (after acceptance)** | ~8 days (T&F official) | Avg. acceptance-to-online-publication (Version of Record) per publisher's journal metrics page; measures online posting, not print issue assignment |

### Publication Frequency

| Aspect | Detail |
|--------|--------|
| **Schedule** | Continuous online / Issue-based (see publisher) |
| **Articles per year (approx.)** | *(see publisher)* |
| **Special issues?** | Yes (themed issues per journal tradition) |

---

## Policies

### Peer Review

| Aspect | Detail |
|--------|--------|
| **Type** | Unclear — T&F's own site self-contradicts for this journal (checked 2026-07-20): the "About this journal" page states "double anonymized... via ScholarOne Manuscripts," while the "Instructions for Authors" page (footer-dated "Updated 22nd June 2026," i.e. more recently and operationally authoritative) states "single anonymous peer reviewed by one independent, anonymous expert" via "Routledge's Submission Portal." Possibly a stale About-page vs. a recently-changed operational policy. Recommend confirming with editorial office before submission; do not assume double-blind norms when preparing anonymization. |
| **Transferable / Cascade?** | *(varies by publisher)* |
| **Reviewer reports published?** | *(varies; some journals offer transparent review)* |
| **Typical R+R rounds** | 1-2 |
| **Reviewer recommendations** | Optional |

### AI Policy

| Aspect | Detail |
|--------|--------|
| **Has journal-specific AI policy?** | Yes (publisher default + may have journal overlay) |
| **Explicit permission gate?** | No — disclosure-based |
| **Leniency (1-5)** | 3-4 |
| **Summary** | AI-assisted writing typically requires acknowledgment; AI listed as author prohibited; verify current policy at journal author guidelines. |
| **Source URL** | https://authorservices.taylorandfrancis.com/editorial-policies/ai-policy/ |

### Preprint Policy

| Stage | Allowed? | Notes |
|-------|----------|-------|
| Pre-submission | Yes | arXiv / bioRxiv / similar permitted by most publishers |
| Under review | Yes | Most journals permit |
| Post-acceptance (AAM) | Yes | Embargo varies by publisher (0-12 months) |
| Version of Record | OA model determines | |

### Open Access

| Aspect | Detail |
|--------|--------|
| **Model** | Hybrid OA |
| **APC (if OA chosen)** | ~$3,420 |
| **Read & Publish agreements** | *(publisher-level; check institutional library)* |

---

## Format

| Aspect | Detail |
|--------|--------|
| **Article types accepted** | Original Research, Case Reports, Reviews, Perspectives |
| **Word limit** | ~8,000 |
| **Word limit negotiability** | Soft (extensions possible) |
| **Abstract limit** | 200 words |
| **Reference limit** | None (typical) |
| **Supplementary material** | Accepted |
| **Figure/Table limits** | *(per article type)* |

---

## Subject Density

> Subject coverage per journal scope. Retrieved 2026-05-17.

### Top Topics (last 5 years)

| Topic | Article Count |
|-------|--------------|
| Psychotherapy Techniques and Applications | 418 |
| Counseling Practices and Supervision | 360 |
| Counseling, Therapy, and Family Dynamics | 247 |
| Child and Adolescent Psychosocial and Emotional Development | 147 |
| Attachment and Relationship Dynamics | 108 |
| Educational and Psychological Assessments | 98 |
| Academic and Historical Perspectives in Psychology | 91 |
| Child Therapy and Development | 70 |
| Mental Health Treatment and Access | 58 |
| Resilience and Mental Health | 52 |

### Orientation

| Dimension | Assessment |
|-----------|------------|
| **Empirical vs. Theoretical** | Primarily empirical |
| **Quantitative vs. Qualitative** | Mixed (per family norm) |
| **Cross-disciplinary openness** | Medium-High within field bounds |

---

## Soft Metadata

> [!WARNING]
> **Tier 2 (community estimate)** — Soft Metadata below adapted from psychology society-flagship family conventions (APA / SAGE / Psychonomic Society / specialty-society norms). Specific journal may diverge; submission practitioners are invited to upgrade individual claims to Tier 1 via `/ja-validate`. See [SEED_DATA_QUALITY.md](../../../SEED_DATA_QUALITY.md) for the Skeleton → Tier 2 → Tier 1 lifecycle.

### Epistemological & Political Leanings

| Dimension | Assessment |
|-----------|------------|
| **Cross-cultural friendliness** | Medium |
| **Non-Western epistemology openness** | Low-Medium |
| **Diversity topic receptiveness** | Medium |
| **Dominant theoretical tradition** | Empirical psychology within society's specialty area; methodological rigor expected |

### Framing Requirements

- **Mandatory framing?** Yes — clear contribution to the society's specialty (vocational / aging / counseling / etc.) required
- **Consequences of ignoring it**: Reviewer pushback for off-specialty work; expect 1-2 R&R rounds with substantive revisions

### Methodological Preferences

| Method | Receptiveness (0-5) | Evidence |
|--------|---------------------|----------|
| Quantitative experimental | 5 | Family norm |
| Qualitative interviews | 3 | Family norm |
| Autoethnography | 1 | Family norm |
| Theoretical / Conceptual | 3 | Family norm |
| Mixed methods | 4 | Family norm |
| Meta-analysis | 4 | 4 article(s) found (OpenAlex keyword search, 2021-2026, 207 total in window; 1.9%) — shows the journal does publish this, but uncommonly — the high rating is a family-level estimate that this output rate does not by itself support |

### Voice & Style

| Aspect | Detail |
|--------|--------|
| **First-person voice acceptance (0-5)** | 2 |
| **Expected density** | Standard APA / society-style |
| **Writing style notes** | APA-style standard; passive voice common in methods/results; first-person increasingly accepted in introduction/discussion |

### Reviewer Pool Characteristics

Society-affiliated specialty community. Reviewer expectations: methodological rigor, specialty-relevance, alignment with society's empirical traditions. R&R cycles common.

| Aspect | Detail | Evidence |
|--------|--------|----------|
| **Dominant tradition** | Empirical psychology within society specialty | Family norm |
| **Reviewer competence variance** | Medium | |
| **Quantitative mindset bias on qualitative work?** | High | |
| **Discourse community signals (editorial board)** | Specialty psychology society leadership | https://www.tandfonline.com/journals/ccpq20 |

### Sensitive Topics

| Topic Category | Receptiveness | Evidence |
|----------------|---------------|----------|
| Human-subjects work | Hard gate (IRB) | Family norm |
| Vulnerable populations (clinical, elderly, etc.) | Heightened ethics scrutiny | Family norm |
| Cross-cultural validity | Increasingly expected | Family norm |
| Pre-registration / Open data | Growing expectation | Family norm |

### Practical Concerns

| Concern | Detail |
|---------|--------|
| **IRB requirement strictness** | Hard gate |
| **Single-author acceptance** | Uncommon |
| **Independent scholar friendliness** | Medium |
| **Data transparency requirement** | Medium-High |
| **OPSEC compatibility** | Medium |

### AI-Research Notes (supplementary AI research, 2026-07-13)

> Added as a **supplementary, independent research pass** — it does NOT overwrite or supersede the Tier assessment above. Treat conflicts as a signal to re-verify, not as an automatic correction. Overall `signal_quality` for this pass: **2/5**.

**AI-research finding (policy):** 此為 Taylor & Francis 出版社通用政策(未查得 CPQ 期刊專屬 AI 頁面,故視為適用但非期刊獨有)。允許生成式 AI 用於研究方法(需遵循 CONSORT-AI/TRIPOD-AI 等報告規範)、文獻回顧/統合分析、語言潤飾、Plain Language Summary。強制揭露:須聲明工具名稱與版本、確認原創性與準確性、確認已檢視工具使用條款、作者對全文完整性(含參考文獻正確性)負全責,並須保留提示詞(prompt)紀錄以備查。AI 工具不得列為作者。同源政策亦見 https://newsroom.taylorandfrancisgroup.com/expanded-guidance-on-ai-application-for-authors-editors-and-reviewers/ 。

**AI-research finding (positioning — what the journal accepts now):** 近期(2025下半-2026)實際刊出內容顯示:量化研究(ROC curve 分析、心理計量量表驗證如 Maladaptive Daydreaming Scale-16 於越南樣本、網絡分析 network analysis)、質化研究(現象學/主題分析式的病人經驗研究,如子宮內膜異位症 CBT 質化研究、治療終止之病人經驗)、概念性/文獻回顧報告(Report 類文章,如去殖民化諮商心理學教育者實踐之概念性回顧)、實務導向的介入效果研究(大學諮商服務成效、e-learning 訓練成效)。Vol 39 Issue 1(2026)為「Decolonizing Counselling Psychology」去殖民化特輯;近期亦刊出多篇涉及文化回應督導(culturally responsive supervision)、少數群體/戰時身份定位(如以色列阿拉伯女性教育諮商師)等社會正義/多元文化取向文章,顯示編輯部目前明顯歡迎批判/去殖民/文化回應框架的投稿。

**AI-research finding (experiential):** 官方期刊指標頁(tandfonline about-this-journal)列出:平均投稿至首次決定 11 天、平均投稿至首次審後決定 125 天、平均接受至線上發表 8 天、接受率 21%(數據為 T&F 官方公佈,非第三方論壇一手經驗,故信號品質中等而非高)。 

**Sources cited in this pass:**
- https://authorservices.taylorandfrancis.com/editorial-policies/using-ai-in-your-research-and-manuscript-preparations/
- https://www.tandfonline.com/journals/ccpq20/about-this-journal (Aims and scope 官方文字)
- https://www.tandfonline.com/journals/ccpq20/about-this-journal (Journal metrics: Speed/acceptance 區塊,擷取數字:11 days avg first decision / 125 days avg first post-review decision / 8 days avg acceptance-to-online / 21% acceptance rate)
- https://www.tandfonline.com/toc/ccpq20/current (Vol 39 Issue 2, 2026 目錄,逐篇標題與方法類型)

**Fields no evidence was found for:**
- **ai_policy (journal-specific)**: 查無 Counselling Psychology Quarterly 或 ccpq20 頁面上獨立列出的 AI 政策;僅有 Taylor & Francis 集團通用 AI 政策頁,推定適用於本刊但未見期刊層級的額外限制或放寬聲明。
- **preprint.source_url (journal-specific)**: 找到的是 Taylor & Francis 集團通用 preprint/green OA 政策(AOM 可上傳非商業性 preprint server,不視為重複發表),未查得 CPQ 專屬 preprint 頁面確認是否有例外。
- **experiential.desk_reject_pct**: SciRev 查無 Counselling Psychology Quarterly 條目(搜尋與嘗試進入 scirev.org/journal/counselling-psychology-quarterly/ 均未取得資料),故桌拒率無法取得,不編造數字。
- **experiential.reviewer_culture**: 小木虫(muchong.com)、知乎(zhihu)、fabiaoji 均搜尋不到針對本刊的具體投稿/審稿討論串;Reddit r/AskAcademia 搜尋亦零結果。無第一手審稿人風格描述來源,故此欄留空,不做主觀推論。
- **experiential.desk_reject_pct 之外的 SciRev 質化評論**: SciRev 平台對此刊沒有使用者評論可供擷取數字,非藏匿不查,而是平台本身無資料。

**Cross-language checks performed:**
- 小木虫 muchong.com — WebSearch 站內/相關關鍵字查詢 0 筆與本刊直接相關的投稿討論
- 知乎 zhihu.com — 搜尋 '咨询心理学季刊'/期刊名相關關鍵字,僅命中泛用心理學 SSCI 期刊彙總文章,未見針對 Counselling Psychology Quarterly 的具體討論串;嘗試 WebFetch 其中一篇彙總文章遭 403 無法確認內文是否列出本刊審稿速度
- fabiaoji — WebSearch 查詢 0 相關結果
- Reddit r/AskAcademia — WebSearch 'reddit AskAcademia "Counselling Psychology Quarterly"' 0 結果
- SciRev — 嘗試直接進入 scirev.org/journal/counselling-psychology-quarterly/ 頁面失敗(導覽被阻擋),WebSearch 亦未返回該刊專屬 SciRev 頁面或評論數據,判定為平台無此刊資料

---

## Strategic Notes

### Hard Blockers

- *(community estimate; submission experience welcome)*

### Soft Tax

Variable; depends on closeness of fit between manuscript framing and the journal's scope.

### Best Suited For

- Counseling psychology research with both quantitative and qualitative methods welcome
- Cross-cultural counseling contributions
- Counselor education / training research
- Authors comfortable with counseling-applied (not pure-academic) framing

### Not Recommended For

- Pure clinical psychology without counseling-applied angle
- Pure intervention efficacy trials (consider clinical psych journals)
- Counseling-adjacent organizational / I-O work

### Rejection Fallback Chain

| Rank | Journal | Why this fallback works |
|------|---------|------------------------|
| 1 | [Journal of Counseling Psychology](./journal-of-counseling-psychology.md) | APA counseling psychology flagship |
| 2 | *(British Journal of Guidance and Counselling — community contribution welcome)* | T&F UK counseling sister |
| 3 | Psychology and Psychotherapy: Theory, Research and Practice | BPS counseling peer (not yet in Atlas) |

---

## Changelog

| Date | Change | By |
|------|--------|----|
| 2026-07-27 | Evidence-citation pass: attached OpenAlex counts to 1 previously uncited high score(s) (Meta-analysis). Counts corroborate presence only — the 0-5 level remains a family-level estimate. Restricted to categories verified as reliably keyword-countable; see scripts/spine/cite_uncited_high_scores.py. | @Zaious (AI-assisted) |
| 2026-07-21 | Added a numeric Top Topics table (10 topics, OpenAlex Source topic counts) — section previously had no numeric rows, so fit_score.py's topic-density scoring defaulted to neutral for this entry regardless of match quality. | @Zaious (AI-assisted) |
| 2026-07-20 | Resolved 3 flagged conflicts (existing vs. AI-research pass): (1) Peer Review Type updated Single-blind → Double-anonymized, verified live against publisher's own journal page; (2) Time to publication (after acceptance) updated `2-6 weeks` estimate → `~8 days` T&F official figure, consistent with sibling metrics already sourced from the same journal-metrics page; (3) Soft Metadata epistemological/political-leaning conflict left for human review (out of scope for direct edit; see AI-Research Notes positioning finding above). | @Zaious (agent-assisted) |
| 2026-07-13 | Added AI-research AI-research supplement (see Soft Metadata > AI-Research Notes) — independent research pass, does not alter existing Tier assessment. | @Zaious |
| 2026-05-17 | Initial entry — v1.3 schema, Tier 2 (community estimate). Soft Metadata adapted from psychology society-flagship family conventions (APA / SAGE / Psychonomic Society / specialty-society norms). | @Zaious |
