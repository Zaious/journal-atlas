<!-- schema: v1.3 -->

# Nature Physics

> **Last verified**: 2026-05-17
> **Maintainer**: @Zaious
> **Venue type**: Journal

---

## Identity

| Field | Value |
|-------|-------|
| **Abbreviation** | Nat. Phys. |
| **Venue type** | Journal |
| **Publisher** | Nature Portfolio (Springer Nature) |
| **ISSN (Print)** | 1745-2473 |
| **ISSN (Online)** | 1745-2481 |
| **Founded** | 2005 |
| **URL** | https://www.nature.com/nphys/ |
| **Author / Submission Guidelines** | https://www.nature.com/nphys/for-authors/preparing-your-submission |
| **Society / Association** | Nature Portfolio family |
| **Editorial Board Location** | London, UK |

---

## Metrics

> Source: publisher / JCR / community trackers. Retrieved 2026-05-17.

| Metric | Value | Date |
|--------|-------|------|
| **Impact Factor** | 17.6 (2024 JCR) | 2026-05-17 |
| **5-Year IF** | *(see JCR)* | |
| **h-index** | ~300+ | 2026-05-17 |
| **CiteScore** | *(see Scopus)* | |
| **Acceptance Rate** | *(community estimate; varies by year)* | |
| **Desk Rejection Rate** | *(community estimate)* | |
| **Quartile (by field)** | *(see SCImago)* | |

### Review Cycle Time

| Stage | Typical Time | Notes |
|-------|-------------|-------|
| **Time to first decision** | *(community estimate)* | |
| **Time to first review** | *(community estimate)* | |
| **Time to acceptance (total)** | *(community estimate)* | |
| **Time to publication (after acceptance)** | *(typical for OA journals: 2-6 weeks after acceptance)* | |

### Publication Frequency

| Aspect | Detail |
|--------|--------|
| **Schedule** | Continuous online (rolling) |
| **Articles per year (approx.)** | *(see publisher / journal homepage)* |
| **Special issues?** | Yes (themed issues / database issues per journal-specific tradition) |

---

## Policies

### Peer Review

| Aspect | Detail |
|--------|--------|
| **Type** | Single-blind (typical journal default; check publisher policy) |
| **Transferable / Cascade?** | *(varies by publisher)* |
| **Reviewer reports published?** | *(varies; some journals offer transparent review)* |
| **Typical R+R rounds** | 1-2 |
| **Reviewer recommendations** | Optional (most journals accept author suggestions) |

### AI Policy

| Aspect | Detail |
|--------|--------|
| **Has journal-specific AI policy?** | Yes (follows publisher default + may have journal-specific overlay) |
| **Explicit permission gate?** | No — disclosure-based |
| **Leniency (1-5)** | 3-4 |
| **Summary** | AI-assisted writing typically requires acknowledgment; AI listed as author prohibited; verify current policy at the journal's author guidelines. |
| **Source URL** | https://www.nature.com/nature-portfolio/editorial-policies/ai |

### Preprint Policy

| Stage | Allowed? | Notes |
|-------|----------|-------|
| Pre-submission | Yes | arXiv / bioRxiv / similar preprint servers permitted by most publishers |
| Under review | Yes | Most journals permit preprint update; verify journal-specific policy |
| Post-acceptance (AAM) | Yes | Embargo varies by publisher (0-12 months) |
| Version of Record | OA model determines | |

### Open Access

| Aspect | Detail |
|--------|--------|
| **Model** | Hybrid OA |
| **APC (if OA chosen)** | ~$12,290 (2026 Gold OA) |
| **Read & Publish agreements** | *(publisher-level; check institutional library)* |

---

## Format

| Aspect | Detail |
|--------|--------|
| **Article types accepted** | Articles, Letters, Reviews, Perspectives, Commentaries, News & Views |
| **Word limit** | ~3,500 (Articles); 1,500-2,000 (Letters) |
| **Word limit negotiability** | Soft (extensions possible with editor approval) |
| **Abstract limit** | 150 words |
| **Reference limit** | None (or generous; verify per article type) |
| **Supplementary material** | Accepted |
| **Figure/Table limits** | *(per article type; verify author guidelines)* |

---

## Subject Density

> Subject coverage per journal scope statement. Retrieved 2026-05-17.

### Top Topics (last 5 years)

| Topic | Article Count (2020-2025) |
|-------|--------------------------|
| Condensed matter physics | *(community estimate)* |
| Quantum information / Quantum computing | *(community estimate)* |
| Particle physics | *(community estimate)* |
| Soft matter / Biophysics | *(community estimate)* |
| Astrophysics / Cosmology | *(community estimate)* |
| Statistical / Nonlinear physics | *(community estimate)* |

### Orientation

| Dimension | Assessment |
|-----------|------------|
| **Empirical vs. Theoretical** | Primarily empirical with theoretical sections welcomed |
| **Quantitative vs. Qualitative** | Primarily quantitative |
| **Cross-disciplinary openness** | Medium-High within field bounds |

---

## Soft Metadata

> [!WARNING]
> **Tier 2 (community estimate)** — Soft Metadata below adapted from Nature Portfolio physics conventions (Nature Physics editorial culture + Nature-family broad-significance norms). Specific journal may diverge; submission practitioners are invited to upgrade individual claims to Tier 1 via `/ja-validate`. See [SEED_DATA_QUALITY.md](../../../SEED_DATA_QUALITY.md) for the Skeleton → Tier 2 → Tier 1 lifecycle.

### Epistemological & Political Leanings

| Dimension | Assessment |
|-----------|------------|
| **Cross-cultural friendliness** | High (international physics community) |
| **Non-Western epistemology openness** | N/A (physics is methodologically Western-anchored but topic-agnostic) |
| **Diversity topic receptiveness** | Medium (diversity initiatives in editorial governance increasing post-2020) |
| **Dominant theoretical tradition** | Experimental / theoretical physics with broad-physics-community significance |

### Framing Requirements

- **Mandatory framing?** Yes (very strong) — must demonstrate broad-physics significance, not just subfield advance
- **Consequences of ignoring it**: Desk reject if work is narrow-subfield without conceptual reach; ~80% desk rejection rate

### Methodological Preferences

| Method | Receptiveness (0-5) | Evidence |
|--------|---------------------|----------|
| Quantitative experimental | 5 | Family norm |
| Qualitative interviews | 0 | Family norm |
| Autoethnography | 0 | Family norm |
| Theoretical / Conceptual | 5 | Family norm |
| Mixed methods | 1 | Family norm |
| Meta-analysis | 2 | Family norm |

### Voice & Style

| Aspect | Detail |
|--------|--------|
| **First-person voice acceptance (0-5)** | 1 |
| **Expected density** | Low — Nature-family broad-significance opening; figures-heavy |
| **Writing style notes** | Standard physics-paper style; tight 3,500-word format requires careful scoping; LaTeX expected; Nature template strict |

### Reviewer Pool Characteristics

Full-time professional editors handle aggressive initial triage. External reviewers from physics community across subfields. Reviewer expectations: conceptual significance for broad physics audience, technical rigor, cross-subfield accessibility.

| Aspect | Detail | Evidence |
|--------|--------|----------|
| **Dominant tradition** | Physics — multi-subfield | Family norm |
| **Reviewer competence variance** | Low — professional editorial curation | |
| **Quantitative mindset bias on qualitative work?** | N/A (physics is quantitative by nature) | |
| **Discourse community signals (editorial board)** | Nature Portfolio professional editors | https://www.nature.com/nphys/ |

### Sensitive Topics

| Topic Category | Receptiveness | Evidence |
|----------------|---------------|----------|
| Dual-use physics (nuclear, weapons) | Medium — heightened editorial scrutiny | Family norm |
| Ethics / Diversity in physics | Medium — Comments/Perspectives format | Family norm |
| Open data / Code sharing | Medium-High | Family norm |

### Practical Concerns

| Concern | Detail |
|---------|--------|
| **IRB requirement strictness** | N/A for most physics work |
| **Single-author acceptance** | Uncommon — theoretical papers possible |
| **Independent scholar friendliness** | Low (institutional infrastructure typically required) |
| **Data transparency requirement** | Medium-High (data sharing per Nature policy) |
| **OPSEC compatibility** | Medium-High (theoretical / computational data sharing flexible) |

---

## Strategic Notes

### Hard Blockers

- *(community estimate based on family norm; submission experience welcome to refine)*

### Soft Tax

Variable; depends on closeness of fit between manuscript framing and the journal's scope statement.

### Best Suited For

- Conceptually significant physics with broad-physics-community appeal
- Experimental breakthroughs with theoretical context
- Quantum information / computing milestones
- Cross-subfield work bridging condensed matter / biology / information

### Not Recommended For

- Incremental advances in a narrow subfield
- Papers requiring substantial prior subfield expertise to appreciate
- Method-only papers without physical discovery

### Rejection Fallback Chain

| Rank | Journal | Why this fallback works |
|------|---------|------------------------|
| 1 | [Physical Review Letters](./physical-review-letters.md) | APS flagship rapid-communication letters |
| 2 | [Physical Review Research](./physical-review-research.md) | APS OA generalist; broader acceptance |
| 3 | [Nature Communications](../multidisciplinary/nature-communications.md) | Nature Portfolio OA broad-scope sister |

---

## Changelog

| Date | Change | By |
|------|--------|----|
| 2026-05-17 | Initial entry — v1.3 schema, Tier 2 (community estimate). Soft Metadata family-adapted from Nature Portfolio physics conventions (Nature Physics editorial culture + Nature-family broad-significance norms). Identity / Metrics / Format authored from publisher / society / community knowledge. | @Zaious |
