<!-- schema: v1.3 -->

# The Clinical Neuropsychologist

> **Last verified**: 2026-09-01
> **Maintainer**: @Zaious
> **Venue type**: Journal

---

## Identity

| Field | Value |
|-------|-------|
| **Abbreviation** | Clin Neuropsychol |
| **Venue type** | Journal |
| **Publisher** | Taylor & Francis (Routledge) |
| **ISSN (Print)** | 1385-4046 |
| **ISSN (Online)** | 1744-4144 |
| **Founded** | 1987 |
| **URL** | https://www.tandfonline.com/journals/ntcn20 |
| **Author / Submission Guidelines** | https://www.tandfonline.com/action/authorSubmission?journalCode=ntcn20 |
| **Society / Association** | National Academy of Neuropsychology (NAN) |
| **Editorial Board Location** | International |

---

## Metrics

> Source: publisher / JCR / community trackers. Retrieved 2026-05-17.

| Metric | Value | Date |
|--------|-------|------|
| **Impact Factor** | 3.4 (2024 JCR) | 2026-05-17 |
| **5-Year IF** | *(see JCR)* | |
| **h-index** | 129 | 2026-09-01 |
| **CiteScore** | *(see Scopus)* | |
| **Acceptance Rate** | ~30% (publisher self-reported, 2024 metric; unverified — SciRev shows 0 user reviews) | 2026-07-13 |
| **Desk Rejection Rate** | *(community estimate)* | |
| **Quartile (by field)** | *(see SCImago)* | |

### Review Cycle Time

| Stage | Typical Time | Notes |
|-------|-------------|-------|
| **Time to first decision** | ~4 days | Publisher self-reported average (may include desk rejections); not independently verified (SciRev: 0 reviews) |
| **Time to first review** | ~47 days (to post-review first decision) | Publisher self-reported average; not independently verified |
| **Time to acceptance (total)** | *(community estimate)* | |
| **Time to publication (after acceptance)** | *(typical: 2-6 weeks)* | |

### Publication Frequency

| Aspect | Detail |
|--------|--------|
| **Schedule** | Continuous online / Issue-based (see publisher) |
| **Articles per year (approx.)** | *(see publisher)* |
| **Special issues?** | Yes (themed issues per journal tradition) |

---

## Policies

### Peer Review

| Aspect | Detail |
|--------|--------|
| **Type** | Single-blind (typical journal default; check publisher) |
| **Transferable / Cascade?** | *(varies by publisher)* |
| **Reviewer reports published?** | *(varies; some journals offer transparent review)* |
| **Typical R+R rounds** | 1-2 |
| **Reviewer recommendations** | Optional |

### AI Policy

| Aspect | Detail |
|--------|--------|
| **Has journal-specific AI policy?** | No dedicated "AI Policy" section, but TCN's own Instructions for Authors (footer-dated "Updated 22nd June 2026") includes a journal-level submission-checklist item: "Declaration of generative AI use... a statement disclosing whether and how generative AI has been used." Substance follows the Taylor & Francis group-wide AI policy; this is a required journal-level touchpoint, not a separate expanded overlay (verified 2026-07-20 via direct primary-source fetch, bypassing the earlier 403 block). |
| **Explicit permission gate?** | No — disclosure-based |
| **Leniency (1-5)** | 2 (conditional/disclosure-gated — Generative AI use is permitted but T&F's group policy requires disclosure for *any* use, full tool name + version + reason, with no exemption for minor edits e.g. copyediting; AI cannot be listed as author) |
| **Summary** | AI-assisted writing typically requires acknowledgment; AI listed as author prohibited; verify current policy at journal author guidelines. |
| **Source URL** | https://authorservices.taylorandfrancis.com/editorial-policies/ai-policy/ |

### Preprint Policy

| Stage | Allowed? | Notes |
|-------|----------|-------|
| Pre-submission | Yes | arXiv / bioRxiv / similar permitted by most publishers |
| Under review | Yes | Most journals permit |
| Post-acceptance (AAM) | Yes | Embargo varies by publisher (0-12 months) |
| Version of Record | OA model determines | |

### Open Access

| Aspect | Detail |
|--------|--------|
| **Model** | Hybrid OA |
| **APC (if OA chosen)** | ~$3,420 |
| **Read & Publish agreements** | *(publisher-level; check institutional library)* |

---

## Format

| Aspect | Detail |
|--------|--------|
| **Article types accepted** | Research, Brief Reports, Case Reports, Reviews, Position Papers |
| **Word limit** | ~8,000 (Research); ~5,000 (Brief Report) |
| **Word limit negotiability** | Soft (extensions possible) |
| **Abstract limit** | 250 words (structured) |
| **Reference limit** | None (typical) |
| **Supplementary material** | Accepted |
| **Figure/Table limits** | *(per article type)* |

---

## Subject Density

> Subject coverage per journal scope. Retrieved 2026-05-17.

### Top Topics (last 5 years)

| Topic | Article Count |
|-------|--------------|
| Traumatic Brain Injury Research | 895 |
| Dementia and Cognitive Impairment Research | 605 |
| Cognitive Functions and Memory | 206 |
| Traumatic Brain Injury and Neurovascular Disturbances | 201 |
| Cardiac Arrest and Resuscitation | 170 |
| Neurobiology of Language and Bilingualism | 156 |
| Counseling Practices and Supervision | 155 |
| Psychological Testing and Assessment | 153 |
| Spatial Neglect and Hemispheric Dysfunction | 140 |
| Cognitive Abilities and Testing | 120 |

### Orientation

| Dimension | Assessment |
|-----------|------------|
| **Empirical vs. Theoretical** | Primarily empirical |
| **Quantitative vs. Qualitative** | Mixed (per family norm) |
| **Cross-disciplinary openness** | Medium-High within field bounds |

---

## Soft Metadata

> [!WARNING]
> **Tier 2 (community estimate)** — Soft Metadata below adapted from neuroscience / cognitive-neuroscience family conventions (Cerebral Cortex / NeuroImage / J Cog Neurosci norms). Specific journal may diverge; submission practitioners are invited to upgrade individual claims to Tier 1 via `/ja-validate`. See [SEED_DATA_QUALITY.md](../../../../../SEED_DATA_QUALITY.md) for the Skeleton → Tier 2 → Tier 1 lifecycle.

### Epistemological & Political Leanings

| Dimension | Assessment |
|-----------|------------|
| **Cross-cultural friendliness** | Medium-High |
| **Non-Western epistemology openness** | Low-Medium |
| **Diversity topic receptiveness** | Medium |
| **Dominant theoretical tradition** | Empirical cognitive / clinical neuroscience with strong methodological rigor |

### Framing Requirements

- **Mandatory framing?** Yes — clear neuroscience contribution required; behavioral psychology without neural measures less competitive
- **Consequences of ignoring it**: Reviewer scrutiny on methodology (sample size, multiple comparisons, replication); statistical rigor expected

### Methodological Preferences

| Method | Receptiveness (0-5) | Evidence |
|--------|---------------------|----------|
| Quantitative experimental | 5 | Family norm |
| Qualitative interviews | 1 | Family norm |
| Autoethnography | 0 | Family norm |
| Theoretical / Conceptual | 3 | Family norm |
| Mixed methods | 2 | Family norm |
| Meta-analysis | 4 | 17 article(s) found (OpenAlex keyword search, 2021-2026, 529 total in window; 3.2%) — confirms the journal publishes this at a meaningful rate |

### Voice & Style

| Aspect | Detail |
|--------|--------|
| **First-person voice acceptance (0-5)** | 2 |
| **Expected density** | Low — formal scientific prose |
| **Writing style notes** | Standard neuroscience paper; extensive methods (especially imaging analysis pipelines); LaTeX or Word |

### Reviewer Pool Characteristics

Cognitive / clinical neuroscience community. Reviewer expectations: rigorous methodology, appropriate statistical thresholds, multiple-comparisons correction, replication or pre-registration where feasible.

| Aspect | Detail | Evidence |
|--------|--------|----------|
| **Dominant tradition** | Empirical cognitive / clinical neuroscience | Family norm |
| **Reviewer competence variance** | Low-Medium | |
| **Quantitative mindset bias on qualitative work?** | Very High | |
| **Discourse community signals (editorial board)** | Neuroscience research community leadership | https://www.tandfonline.com/journals/ntcn20 |

### Sensitive Topics

| Topic Category | Receptiveness | Evidence |
|----------------|---------------|----------|
| Human-subjects neuroimaging | Hard gate (IRB + consent) | Family norm |
| Clinical populations / vulnerable groups | Heightened consent / ethical scrutiny | Family norm |
| Reproducibility / Open data | High (BIDS / OpenNeuro encouraged) | Family norm |
| Pre-registration | Increasingly expected | Family norm |

### Practical Concerns

| Concern | Detail |
|---------|--------|
| **IRB requirement strictness** | Hard gate for human-subjects work |
| **Single-author acceptance** | Uncommon |
| **Independent scholar friendliness** | Low (imaging infrastructure required) |
| **Data transparency requirement** | High |
| **OPSEC compatibility** | Medium |

### AI-Research Notes (supplementary AI research, 2026-07-13)

> Added as a **supplementary, independent research pass** — it does NOT overwrite or supersede the Tier assessment above. Treat conflicts as a signal to re-verify, not as an automatic correction. Overall `signal_quality` for this pass: **2/5**.

**AI-research finding (policy):** 未見期刊專屬 AI 政策頁,僅有出版社(Taylor & Francis)集團層級政策可查(WebFetch 被 403 擋下,以下轉引自 WebSearch 摘要,非逐字):生成式 AI 不得列為作者;若使用生成式 AI 協助寫作,須在文中揭露工具全名、版本與用途;作者對內容正確性負最終責任。另有搜尋摘要提到「Taylor & Francis 不允許將生成式 AI 用於產生或竄改研究/臨床測試的輸出結果」,但此句來自 AI 搜尋引擎二次摘要而非我方直接讀到原文,故信度打折,標記為 conditional/leniency=2(允許用但要揭露,非全面禁止但也非開放)。找不到 TCN 期刊層級的加嚴或放寬版本。

**AI-research finding (positioning — what the journal accepts now):** 2024-2026 近期刊出/線上優先文章涵蓋:AI 與自然語言處理於臨床神經心理學的敘事回顧 (2025.2547934)、生成式 AI 是否對表現效度測驗(PVT)安全性構成風險 (2024.2379023)、AI 於神經心理臨床實務的工具選擇/隱私/計費指引 (2026 Vol 40 No 3)、Minnesota Conference 神經心理學教育訓練指引 (2025.2520957)、亞裔美國病患族群的神經心理訓練與臨床實務現況 (2025.2523534)、Society for Clinical Neuropsychology 2023 策略計畫調查結果 (2025.2547020)、AACN 年會摘要輯 (2025 Vol39No4)。可見:實證研究、統合分析/系統性回顧、批判性整合回顧、常模/心理計量研究、法醫應用、專業議題(認證/教育訓練/倫理)調查、立場/共識聲明、書評、以及 'Grand Rounds' 單一個案報告(需含病史、醫學/神經影像、詳細神經心理評估結果、鑑別診斷討論)皆為期刊自述可接受類型。AI 相關主題本身正在成為近兩年常見投稿題材(以「AI 如何影響神經心理評估/臨床實務」為框架,而非 AI 生成論文本身)。

**AI-research finding (experiential):** 出版社自報整體接受率約 30%(2024 年度指標)。 

**Sources cited in this pass:**
- https://scirev.org (搜尋 'Clinical Neuropsychologist' 顯示 'Journal – 0 reviews')
- https://taylorandfrancis.com/our-policies/ai-policy/
- https://www.tandfonline.com/doi/abs/10.1080/13854046.2024.2379023
- https://www.tandfonline.com/doi/full/10.1080/13854046.2025.2520957
- https://www.tandfonline.com/doi/full/10.1080/13854046.2025.2523534
- https://www.tandfonline.com/doi/full/10.1080/13854046.2025.2547934
- https://www.tandfonline.com/doi/full/10.1080/13854046.2026.2635117
- https://www.tandfonline.com/journals/ntcn20/about-this-journal
- https://www.tandfonline.com/journals/ntcn20/about-this-journal (T&F 官方 journal metrics 頁,自報數據)

**Fields no evidence was found for:**
- **ai_policy (期刊層級)**: T&F 官網 AI 政策頁對 WebFetch 回傳 403,僅能靠 WebSearch 二次摘要取得政策要點,且找不到 TCN 期刊本身的專屬 AI 政策頁或投稿須知中的 AI 段落(files.taylorandfrancis.com/ntcnauth.pdf 同樣 403 無法直接讀取原文核對)
- **preprint.allowed (期刊層級)**: 僅有 T&F 集團通用政策,未查得 TCN 專屬聲明或 embargo 天數
- **experiential.reviewer_culture**: SciRev 該刊 0 篇評論;搜尋小木虫、知乎、Reddit r/AskAcademia 均未找到針對此刊審稿人風格/文化的具體第一手討論,只有期刊簡介與影響因子類型的轉載內容
- **experiential.desk_reject_pct**: 官方 journal metrics 頁未拆分桌拒率與送外審後拒稿率,僅提供合併的『4 天平均首次決定』與『30% 接受率』,無法反推桌拒比例;SciRev/論壇均無第一手數據

**Cross-language checks performed:**
- 小木虫 muchong.com — 未找到針對 The Clinical Neuropsychologist 的專屬審稿討論帖,搜尋結果只帶出小木虫論壇首頁與其他期刊(如 Nature Communications)的審稿心得,0 個直接相關結果
- 知乎 zhihu.com — 找到 1 篇期刊介紹文 (zhuanlan.zhihu.com/p/685095032),但 WebFetch 被知乎擋下(403)無法讀取內文查證是否含審稿時效/接受率等第一手經驗,WebSearch 摘要僅重複期刊基本資料(IF、創刊年、主編),未見具體投稿經驗
- Reddit r/AskAcademia — 搜尋未返回任何針對本刊的討論串,0 results
- fabiaoji — 未執行到獨立搜尋(中文搜尋結果被小木虫/知乎/一般 SCI 期刊推薦網站佔滿,未見 fabiaoji 站內結果)

---

## Strategic Notes

### Hard Blockers

- *(community estimate; submission experience welcome)*

### Soft Tax

Variable; depends on closeness of fit between manuscript framing and the journal's scope.

### Best Suited For

- Clinical neuropsychology research with strong methodology
- Test validation and normative studies
- Cross-cultural neuropsychological adaptation
- Authors affiliated with NAN community

### Not Recommended For

- Pure basic neuroscience without clinical application
- Single-case reports without broader methodological / clinical contribution
- Cognitive psychology without clinical population angle

### Rejection Fallback Chain

| Rank | Journal | Why this fallback works |
|------|---------|------------------------|
| 1 | [Archives of Clinical Neuropsychology](./archives-of-clinical-neuropsychology.md) | NAN peer journal (OUP); similar audience |
| 2 | [Neuropsychology](./neuropsychology.md) | APA clinical-neuropsychology peer |
| 3 | [Journal of Clinical and Experimental Neuropsychology](./journal-of-clinical-and-experimental-neuropsychology.md) | T&F sister journal with experimental focus |

---

## Changelog

| Date | Change | By |
|------|--------|----|
| 2026-07-27 | Evidence-citation pass: attached OpenAlex counts to 1 previously uncited high score(s) (Meta-analysis). Counts corroborate presence only — the 0-5 level remains a family-level estimate. Restricted to categories verified as reliably keyword-countable; see scripts/spine/cite_uncited_high_scores.py. | @Zaious (AI-assisted) |
| 2026-07-21 | Added a numeric Top Topics table (10 topics, OpenAlex Source topic counts) — section previously had no numeric rows, so fit_score.py's topic-density scoring defaulted to neutral for this entry regardless of match quality. | @Zaious (AI-assisted) |
| 2026-07-20 | Resolved 2 flagged conflicts (AI Policy: journal-overlay existence + leniency score) via live re-verification (WebSearch on T&F group AI policy; WebFetch on authorservices.taylorandfrancis.com AI-policy page and ntcnauth.pdf both still 403). Confirmed no TCN-specific AI policy page exists; adopted leniency=2 reflecting T&F's no-exemption "any use must be disclosed" stance. | @Zaious |
| 2026-07-13 | Added AI-research AI-research supplement (see Soft Metadata > AI-Research Notes) — independent research pass, does not alter existing Tier assessment. | @Zaious |
| 2026-05-17 | Initial entry — v1.3 schema, Tier 2 (community estimate). Soft Metadata adapted from neuroscience / cognitive-neuroscience family conventions (Cerebral Cortex / NeuroImage / J Cog Neurosci norms). | @Zaious |
| 2026-09-01 | Refreshed metrics from OpenAlex (h_index) | @bot |
