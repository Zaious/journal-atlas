...
`E2E_TEST_GUIDE.md` (kept in the maintainer's workspace, not committed, so this is deliberately not a link) defines the six scenarios that every release should pass. Each case study here is anchored to one or more of those scenarios so reviewers can verify the skill's behavior hasn't regressed.
